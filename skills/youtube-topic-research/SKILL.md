...
The scripts ship with this plugin; call them by absolute path so they resolve after install:

- Demand: `${CLAUDE_PLUGIN_ROOT}/scripts/trends_serpapi.py`
- Competition: `${CLAUDE_PLUGIN_ROOT}/scripts/youtube_competition.py`

Run them with `python` — or `python3` on machines where that's the command name (Python 3.7+,
...
```
python "${CLAUDE_PLUGIN_ROOT}/scripts/trends_serpapi.py" --terms "a,b,c,d,e" --timeframe "today 3-m" --gprop youtube
```
...
```
python "${CLAUDE_PLUGIN_ROOT}/scripts/youtube_competition.py" --terms "a,b" --max 25
```
...
Store them where the scripts read them — real environment variables take precedence over any
file, then a `.env` in the folder you're working in, then `~/.trend-research-toolkit/.env`:
...
Don't keep keys inside the plugin directory — it's a managed cache that's replaced on update.
