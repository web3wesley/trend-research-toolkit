...
## Where the scripts are (self-contained global skill)

The Python scripts are bundled **inside this skill's own folder**. Run them by their full path
(expand `~` to the home directory; use `python3` where that's the interpreter name — Python
3.7+, standard library only, no install step):
